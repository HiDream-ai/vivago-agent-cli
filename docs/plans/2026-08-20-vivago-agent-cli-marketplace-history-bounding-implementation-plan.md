...
| 5 公司 Beta 工作流 | 代码已完成，未启用 | 已替换旧快进逻辑；公司安装分支尚无保护，不能发布 |
| 6 文档和回归 | 进行中 | 待全量回归和提交同步；公司仓库规则由管理员另行配置 |
...
前置条件：阶段 4 全部通过，并确认公司仓库规则允许发布 Workflow 对 `marketplace` 使用受控强推，
同时不开放人员直接强推 `main`、Tag 或安装分支。
...
当前实现已经完成上述工作流修改和契约测试。2026-08-20 只读检查显示公司仓库 ruleset 为空，
`marketplace` 未启用分支保护，尚不满足“Workflow 可受控强推、人员不可直接改写”的要求。正式启用
还差仓库管理员配置规则、代码进入公司 `main` 以及下一次 Beta 发布窗口；本次不运行
`Publish Beta`。
...
外部等待时间主要来自六平台 Workflow 和公司仓库规则确认，不计入纯开发时间。
