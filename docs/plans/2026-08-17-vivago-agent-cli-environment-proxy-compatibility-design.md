...
同日使用 `-tags prod` 的当前 macOS ARM64 构建补充了真实生产验证。本机同时存在标准环境代理和
Clash TUN；普通 API 可以访问生产环境，但对象存储域名最初命中 Google 通用代理规则，两个代理节点
都在 TLS 阶段关闭连接。全局直连会中断 Codex，因此没有把“切全局网络模式”作为使用要求，也没有
在 CLI 中增加第三方代理软件适配。

本机只给对象存储域名增加精确直连规则后，默认终端代理环境和 TUN 路径均能建立 TLS。生产 CLI
随后完成附件上传、服务端读取、SSE、图片生成、`RUN_FINISHED`、preview 和 download，预览与下载
文件字节数一致。这个结果同时说明两点：`ProxyFromEnvironment` 改造仍然用于支持只配置标准环境
代理的普通用户；第三方代理对具体域名的错误分流属于本机网络配置，不应写进插件或绕过现有安全校验。
