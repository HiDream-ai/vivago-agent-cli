...
实现将上传和下载分别拆成安全直连 Transport 与环境代理 Transport，由请求级选择器使用
`http.ProxyFromEnvironment` 路由。代理路径在连接用户代理前校验业务目标，直连路径继续固定到已校验
公网 IP。下载网络错误改为稳定的脱敏错误，不再包装可能带有代理凭证的底层错误文本。

2026-08-17 完成以下验证：

- 代理选择测试先在原实现上失败，报错为安全拨号器拒绝本地代理；实现分流后转为通过；
- 上传和下载完整包测试通过；
- `go test ./...` 与 `go test -tags prod ./...` 通过；
