...
首次 Publish Beta 曾因临时回滚测试继承全局 `commit.gpgsign` 配置而失败；修复后以提交 `d29fdc7`
重新运行并全绿。`v0.3.0-beta.1` 已创建为 GitHub Prerelease，生产 `marketplace` 分支已更新，
并上传 Marketplace 压缩包、SHA256SUMS 和 SPDX SBOM。
