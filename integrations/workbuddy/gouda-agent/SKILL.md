...
如果命令输出 `payment_required`，说明本次 Turn 因积分或会员等级不足而失败。Skill 同时兼容国内
接口返回的数字错误码和 AgentOS 公共 `RUN_ERROR` 错误码：积分不足包括 `2007` 或
`insufficient_credits`，会员等级不足为 `2020`：

- `reason=credits` 时打开够搭积分商品页；`reason=membership` 时打开够搭会员商品页；
- 告知用户支付页面是否已自动打开，并在 `opened=false` 时提供输出中的 `url` 手动打开；
- 支付完成后让用户回到输出中的 `return_url`（够搭首页）完成网页提示的支付确认；
- 等用户明确确认支付完成后，才在原 Conversation 中重新提交后续任务，不要自动重复提交或声称支付成功。
...
- 支付门槛导致的 `payment_required` 不是网络中断，不能使用 `resume` 重试原 Turn；支付确认后应在
  原 Conversation 中创建新的 Turn。
